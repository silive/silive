...
async function main() {
  ...
  try {
    ...
    const schemaBeforeNegativeGates = await schemaSnapshot(pool)
    await assert.rejects(() => runMigrations({ argv: [...commonRehearsal, `--preflight-plan=${preflightPlan}`, `--preflight-plan-sha256=${"0".repeat(64)}`, `--backup-manifest=${backupManifest}`, `--operation-log=${operationLog}`], env, logger: SILENT }), /计划摘要/)
    check("03c 迁移计划摘要错误零写入拒绝", JSON.stringify(await schemaSnapshot(pool)) === JSON.stringify(schemaBeforeNegativeGates))
    const changedPlan = path.join(tempDir, "changed-preflight-plan.json")
    const changedPayload = JSON.parse(fs.readFileSync(preflightPlan, "utf8")); changedPayload.migrations[0].sha256 = "0".repeat(64); fs.writeFileSync(changedPlan, JSON.stringify(changedPayload))
    await assert.rejects(() => runMigrations({ argv: [...commonRehearsal, `--preflight-plan=${changedPlan}`, `--preflight-plan-sha256=${guard.sha256File(changedPlan)}`, `--backup-manifest=${backupManifest}`, `--operation-log=${operationLog}`], env, logger: SILENT }), /迁移文件/)
    check("03d 迁移文件摘要篡改零写入拒绝", JSON.stringify(await schemaSnapshot(pool)) === JSON.stringify(schemaBeforeNegativeGates))
    await assert.rejects(() => runMigrations({ argv: [...commonRehearsal, `--preflight-plan=${preflightPlan}`, `--preflight-plan-sha256=${planSha256}`, `--backup-manifest=${path.join(tempDir, "missing-manifest.json")}`, `--operation-log=${operationLog}`], env, logger: SILENT }), /备份清单/)
    check("03e 缺失备份清单零写入拒绝", JSON.stringify(await schemaSnapshot(pool)) === JSON.stringify(schemaBeforeNegativeGates))
    await assert.rejects(() => runMigrations({ argv: [...commonRehearsal, `--preflight-plan=${preflightPlan}`, `--preflight-plan-sha256=${planSha256}`, `--backup-manifest=${backupManifest}`, `--operation-log=${operationLog}`], env: { ...env, VSC_GUARD_SIMULATE_AVAILABLE_BYTES: "1" }, logger: SILENT }), /磁盘空间不足/)
    check("03f 磁盘空间不足零写入拒绝", JSON.stringify(await schemaSnapshot(pool)) === JSON.stringify(schemaBeforeNegativeGates))
    await assert.rejects(() => runMigrations({ argv: [...commonRehearsal, `--preflight-plan=${preflightPlan}`, `--preflight-plan-sha256=${planSha256}`, `--backup-manifest=${backupManifest}`, `--operation-log=${operationLog}`], env: { ...env, AI_PREVIEW_ENABLED: "true" }, logger: SILENT }), /AI_PREVIEW_ENABLED/)
    check("03g AI预览开启零写入拒绝", JSON.stringify(await schemaSnapshot(pool)) === JSON.stringify(schemaBeforeNegativeGates))
    ...
    const cleanupBeforeNegativeGates = await orderSnapshot(pool, [...TEST_ORDERS, ...NORMAL_ORDERS])
    await assert.rejects(() => runCleanup({ argv: [...commonRehearsal, "--apply", "--confirm-delete-test-orders", "--confirm-exact-count=10", `--whitelist-file=${whitelistFile}`, `--cleanup-plan=${cleanupPlan}`, `--cleanup-plan-sha256=${"0".repeat(64)}`, `--backup-manifest=${backupManifest}`, `--operation-log=${operationLog}`], env: cleanupEnv, logger: SILENT }), /计划摘要/)
    check("20a 清理计划摘要错误零写入拒绝", JSON.stringify(await orderSnapshot(pool, [...TEST_ORDERS, ...NORMAL_ORDERS])) === JSON.stringify(cleanupBeforeNegativeGates))
    fs.writeFileSync(whitelistFile, JSON.stringify({ orderIds: [...TEST_ORDERS.slice(0, 9), NORMAL_ORDERS[0]] }))
    await assert.rejects(() => runCleanup({ argv: [...commonRehearsal, "--apply", "--confirm-delete-test-orders", "--confirm-exact-count=10", `--whitelist-file=${whitelistFile}`, `--cleanup-plan=${cleanupPlan}`, `--cleanup-plan-sha256=${guard.sha256File(cleanupPlan)}`, `--backup-manifest=${backupManifest}`, `--operation-log=${operationLog}`], env: cleanupEnv, logger: SILENT }), /白名单文件摘要/)
    check("20b 白名单变更零写入拒绝", JSON.stringify(await orderSnapshot(pool, [...TEST_ORDERS, ...NORMAL_ORDERS])) === JSON.stringify(cleanupBeforeNegativeGates))
    fs.writeFileSync(whitelistFile, JSON.stringify({ orderIds: TEST_ORDERS }))
