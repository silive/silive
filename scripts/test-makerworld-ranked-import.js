...
assert.match(admin, /data-product-scope="selling">在售商品/)
assert.match(admin, /data-product-scope="review">导入审核/)
assert.match(admin, /isApprovedImportedProduct\(item\)/)
assert.match(admin, /导入商品仅保留一张主图/)
assert.doesNotMatch(admin, /data-product-status=/)
assert.match(server, /galleryImages: modelCandidateId \? \[\]/)
