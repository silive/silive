...
function addressErrorMessage(error = {}) {
  ...
  if (text.includes("deny") || text.includes("auth")) return "请在系统设置中允许使用通讯地址，或手动填写"
  if (text.includes("permission") || text.includes("scope") || text.includes("no permission")) return "当前小程序暂不支持通讯地址，请手动填写"
  return "无法打开通讯地址，请手动填写"
}
